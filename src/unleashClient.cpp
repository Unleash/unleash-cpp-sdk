...
UnleashClient::UnleashClient(ClientConfig p_config, Context p_ctx)
    ...
    _context(std::move(p_ctx)), 
    _metricStore(_config), 
    _metricSender(_config),
    _eventHandler(std::make_shared<EventHandler>()),
    _toggleFetcher(_config)
{ 
...
void UnleashClient::stop()
{
    ...
    if (_mSendingThread.joinable()) _mSendingThread.join();
    if (_fPollingThread.joinable()) _fPollingThread.join();
...
void UnleashClient::initializeToggleCache()
{
    ...
    if (_config.bootstrapOverride() 
        && bootstrap.has_value() 
        && !bootstrap->getToggles().empty() ) 
    {
        const ToggleSet bootstrapToggles(bootstrap->getToggles());
        _flagStore.replace(std::make_shared<ToggleSet>(bootstrapToggles));
        bootstrapValid = true;
    ...
    {
        if(bootstrapValid) persistToggles(ToggleSet{bootstrap->getToggles()});
    }
}
...
{
    if (!_config.isRefreshEnabled()) return;
    ...
    while (!_exitThreads.load(std::memory_order_acquire))
    {
        Context snapshot = [&]{
            ...
            _cvPolling.wait_for(lock, interval, [this] {
                return _exitThreads.load(std::memory_order_acquire) || _contextUpdated;
            });
...
void UnleashClient::singleFetchToggles(const Context& p_ctx)
{
    ...
    if(fetchResult.error.has_value())
    {
        ...
        // emit onError(...)
        _eventHandler->emitError(EventHandler::ClientError{
            "Toggle fetch failed",
            std::move(fetchResult.error.value())
        });
    ...
    if ((fetchResult.status >= utils::httpStatusOkLower && 
        fetchResult.status < utils::httpStatusOkUpper))
    {
        if(fetchResult.toggles.has_value())
        {
            ...
            _flagStore.replace(
                    std::make_shared<unleash::ToggleSet>(std::move(fetchResult.toggles.value()))
                    );
...
{
    if (!_config.isMetricsEnabled()) return;

    auto waitAndMaybeSend = [&](std::chrono::milliseconds timeout) -> bool {
        std::unique_lock<std::mutex> lock(_mutexMetrics);
        _cvMetrics.wait_for(lock, timeout, [this] {
            return _exitThreads.load(std::memory_order_acquire);
        });

        if (_exitThreads.load(std::memory_order_acquire)) return false; 
    ...
    if (initialDelay.count() > 0) {
        if (!waitAndMaybeSend(initialDelay)) return;
    } 

    // Periodic phase
    while (!_exitThreads.load(std::memory_order_acquire)) {
        if (!waitAndMaybeSend(interval)) break;
    }
}
...
bool UnleashClient::isEnabled( const std::string& flagName)
{
    ...
    if(!toggleSet || !toggleSet->contains(flagName)) return false;
    ...
    if(this->_config.impressionDataAll() || impression)
    {
        ...
        _eventHandler->emitImpression(unleash::EventHandler::ClientImpression{
            ctx, 
            flagName, 
            enabled, 
            "isEnabled", 
...
{
    if(!this->isReady()) return std::nullopt;
    auto toggleSet = _flagStore.snapshot();
    if(!toggleSet || !toggleSet->contains(flagName)) return std::nullopt;
    bool enabled =  toggleSet->isEnabled(flagName);
